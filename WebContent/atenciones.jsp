       					 ...
       					 <%if(request.getAttribute("listFicha") != null){
       	            		...
       	            		for(FichaLente f: listPac){
       						 ...
       						 %>
           							...
           							<tr class="<%=tipo%>">
           								<td><button type="button" onclick="document.location ='/ModificarLente?hc=<%=((Paciente)request.getAttribute("pac")).getApellido()%>&hc=?<%=f.getIdFicha()%>'" class="btn btn-outline btn-warning text-right btn-block"><%=f.getIdFicha()%></button></td>
